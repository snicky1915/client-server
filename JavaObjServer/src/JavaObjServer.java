...
//				Login();
		...
		public void Login() {
			...
			for (int i = 0; i < user_vc.size(); i++) {
				...
				ChatMsg cm = new ChatMsg(user.UserName, "100", "");
				if (user != this)
					WriteOneObject(cm);
			}
			...
			ChatMsg cm = new ChatMsg(UserName, "100", msg);
		...
		public void Logout() {
			...
			ChatMsg cm = new ChatMsg(UserName, "101", "");
			WriteAllObject(cm);
		}
...
//					msg = msg.trim(); // 앞뒤 blank NULL, \n 모두 제거
					...
					if (cm.getCode().matches("100")) {
						...
						Logout();
						break;
					} else if (cm.getCode().matches("300")) { // 파일 보내 Gui 
						WriteAllObject(cm);
					}
				} catch (IOException e) {
